...
with open("GeneratedDocs.md", "w", encoding="UTF-8") as f:
    ...
    f.write(
        f"[![Add repo `{repo['id']}`](https://img.shields.io/badge/Add_repo-{repo['id']}-blue)](https://gterminal.js.org/?cmd=modules+add-repo+{quote(repo['address'])})\n"
    )
    f.write(
        f"[![Remove repo `{repo['id']}`](https://img.shields.io/badge/Remove_repo-{repo['id']}-blue)](https://gterminal.js.org/?cmd=modules+remove-repo+{repo['id']})\n"
    )
