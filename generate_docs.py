...
with open("GeneratedDocs.md", "w", encoding="UTF-8") as f:
    ...
    for id, module in repo["modules"].items():
        f.write(f"\n## [`{id}`] {module['name']}\n\n")
        f.write(f"[![Install module `{repo['id']}:{id}`](https://img.shields.io/badge/Install_module-{repo['id']}:{id}-blue)](https://gterminal.js.org/?cmd=modules+install+{repo['id']}:{id})\n")
        f.write(f"[![Remove module `{repo['id']}:{id}`](https://img.shields.io/badge/Remove_module-{repo['id']}:{id}-blue)](https://gterminal.js.org/?cmd=modules+remove+{repo['id']}:{id})\n\n")
